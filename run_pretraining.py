...
def run_validation_ep(agent, env, target):
    obs = env.reset()
    done = False
    while not done:
        w_obs = env.set_action_m(target)
        action = agent.get_action(w_obs)
        _obs, _, done, _ = env.step(np.array(action))
        obs = _obs.manager
...
def main(args):
    ...
    for step in tqdm(range(total_training_steps), smoothing=0.01):
        if args.training_val_frequency and step % args.training_val_frequency == 0:
            val_key, _ = jax.random.split(val_key)
        ...
        buffer.add(w_obs, action, 0.0, reward.workers[0], terminal_state, _obs.workers[0])
